...
export default function PublicSAKIPPage() {
  ...
  const avgNilai =
    totalOPD > 0
      ? nilaiList.reduce((sum, n) => sum + n.nilai_total, 0) / totalOPD
      : 0;
  const predikatA = nilaiList.filter(
    (n) => n.predikat === "A" || n.predikat === "AA",
  ).length;
  const predikatBB = nilaiList.filter(
    (n) => n.predikat === "BB" || n.predikat === "B",
  ).length;

  const years = Array.from(new Set(nilaiList.map((n) => n.tahun))).sort(
    (a, b) => b - a,
  );
  ...
  const topScore = [...filteredNilai].sort(
    (a, b) => b.nilai_total - a.nilai_total,
  )[0];
    ...
    <div className="relative min-h-screen overflow-hidden bg-[radial-gradient(circle_at_top_left,rgba(39,81,191,0.14),transparent_24%),radial-gradient(circle_at_bottom_right,rgba(232,183,35,0.18),transparent_22%)] py-8 sm:py-10">
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="space-y-8">
              ...
              <div className="space-y-6 animate-in fade-in-0 slide-in-from-top-4 duration-700">
                <div className="max-w-3xl space-y-4">
                  <h1 className="text-3xl font-semibold tracking-tight text-foreground sm:text-4xl lg:text-5xl">
                    Pantau nilai, predikat, dan dokumen SAKIP dengan ritme
                    visual yang lebih jelas.
                  </h1>
                  <p className="text-base leading-7 text-muted-foreground sm:text-lg">
                    Halaman ini merangkum performa OPD, memudahkan penyaringan
                    data, dan menampilkan detail penilaian dengan bahasa visual
                    yang konsisten dengan portal publik lainnya.
                  </p>
                </div>
                ...
                <CardContent className="space-y-6 p-6 sm:p-7">
                  ...
                  <div className="rounded-3xl border border-white/10 bg-white/5 p-5">
                    <p className="text-sm text-slate-400">
                      Nilai tertinggi pada hasil tersaring
                    </p>
                  ...
                  <div className="rounded-3xl border border-white/10 bg-white/5 p-5">
                    <div className="flex items-center gap-3">
                      ...
                      <p className="text-sm text-slate-300">
                        Penilaian, dokumen, dan detail komponen kini bisa dibaca
                        lebih cepat tanpa kehilangan konteks data.
                      </p>
                    </div>
                  </div>
                </CardContent>
                ...
                <CardContent className="space-y-5 p-5">
                  ...
                  <div className="rounded-2xl border border-dashed border-border/80 bg-muted/30 p-4">
                    ...
                    <p className="mt-1 text-sm text-muted-foreground">
                      {filteredNilai.length} nilai dan {filteredDokumen.length}{" "}
                      dokumen tampil
                    </p>
                                    ...
                                    <TableCell className="text-center">
                                      <Badge
                                        className={cn(
                                          ...
                                          getPredikatColor(nilai.predikat),
                                        )}
                                      >
                                        {nilai.predikat}
                                      </Badge>
          ...
          <Dialog
            open={!!selectedOPD}
            onOpenChange={() => setSelectedOPD(null)}
          >
                ...
                <div className="space-y-6">
                  <div
                    className={cn(
                      ...
                      getPredikatSurface(selectedOPD.predikat),
                    )}
                  >
                    <div>
                      <p className="text-sm text-muted-foreground">
                    ...
                    <Badge
                      className={cn(
                        ...
                        getPredikatColor(selectedOPD.predikat),
                      )}
                    >
                  ...
                  <div className="grid gap-4 sm:grid-cols-5">
                    {[
                      {
                        label: "Perencanaan",
                        value: getKomponen(selectedOPD).perencanaan,
                        max: 30,
                      },
                      {
                        label: "Pengukuran",
                        value: getKomponen(selectedOPD).pengukuran,
                        max: 25,
                      },
                      {
                        label: "Pelaporan",
                        value: getKomponen(selectedOPD).pelaporan,
                        max: 15,
                      },
                      {
                        label: "Evaluasi",
                        value: getKomponen(selectedOPD).evaluasi,
                        max: 10,
                      },
                      {
                        label: "Capaian",
                        value: getKomponen(selectedOPD).capaian,
                        max: 20,
                      },
                    ].map((item) => (
                      <div
                  ...
                  <div className="flex items-center justify-between text-sm text-muted-foreground">
                    ...
                    <span>
                      {selectedOPD.created_at
                        ? new Date(selectedOPD.created_at).toLocaleDateString(
                            "id-ID",
                            {
                              day: "numeric",
                              month: "long",
